...
const checkout = async (req, res) => {
  try{
    ...
    const id = req._id
    const courseData =  await courseModel.findOne({_id:courseId},'title instructorId price promotion imageURL -_id').lean()
    ...
    const discount = !courseData.promotion? 0 : (new Date(courseData.promotion.saleEndDate)> new Date())? courseData.promotion.discount:0
    let coupon = {_id:50}
...
const createPayment = async (req, res) => {
    ...
    const {courseId} = req.body
    const id = req._id
    const courseData =  await courseModel.findOne({_id:courseId},'price promotion -_id').lean()
    let price =courseData.price
    if(courseData.promotion) price = (new Date(courseData.promotion.saleEndDate)<new Date())? courseData.price:courseData.price-courseData.promotion.discount/100*courseData.price
    ...
    if(price<=wallet){
      ...
      res.status(200).json({method:'mixed',client_secret:paymentIntent.client_secret,paymentIntentId:paymentIntent.id})
    }
  }
    catch(err){
      console.log(err);
...
const payWithWallet = async (req, res) => {
  try{
    const {courseId} = req.body
    const id = req._id
    const courseData =  await courseModel.findOne({_id:courseId},'price promotion instructorId -_id').lean()
    let price =courseData.price
    if(courseData.promotion) price = (new Date(courseData.promotion.saleEndDate)<new Date())? courseData.price:courseData.price-courseData.promotion.discount/100*courseData.price
  ...
  try{
    ...
    if(wallet===0){
      await stripe.paymentIntents.retrieve(paymentIntentId).then((payment)=>{
        console.log(payment);
        console.log(payment.metadata.courseId.toString(),courseId.toString(),payment.status);
        console.log(payment.amount_received,price*1000,payment.amount_received === price*1000);
        if(payment.metadata.courseId.toString()===courseId.toString() && payment.status==='succeeded'){
          const execute = async () =>{
            try{
              // collect the price first
      ...
      res.status(400).json({message: "You wallet is not empty"})
    }
...
const checkBeforeProceed = async (req, res) => {
  try{
    ...
    const wallet = (traineeData.wallet || 0)
    await stripe.paymentIntents.retrieve(paymentIntentId).then((payment)=>{
      if(payment.metadata.courseId.toString()===courseId.toString()){
        if(payment.metadata.method==='card'){
          if(wallet===0){
        ...
        else if(payment.metadata.method==='mixed'){
