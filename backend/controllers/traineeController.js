...
const { trainee, courseRecordModel } = require('../models/traineeModel')
...
const addCourseToTrainee = async (req, res) => {
    try {
        const traineeId = await req.params.id
        const { courseId } = req.body
        const newCourseRecord = await new courseRecordModel({ courseId: courseId })
        const newCourseList = await trainee.findOneAndUpdate({ _id: traineeId }, { $addToSet: { courseList: newCourseRecord } }, { new: true, upsert: true })
...
const addLessonRecord = async (req, res) => {
    try {
        ...
        const newCourseList = await trainee.updateOne({ _id: traineeId, 'courseList.courseId': courseId }, { $push: { 'courseList.$.lessonsList': { lessonId, note: "" } } })
        res.status(200).json({ newCourseList })
    } catch (err) {
        res.status(400).json({ error: err.message })
    }
...
const addExerciseRecord = async (req, res) => {
    try {
        ...
        const newCourseList = await trainee.updateOne({ _id: traineeId, 'courseList.courseId': courseId }, { $push: { 'courseList.$.exercisesList': { exercisesId: exerciseId, grade: grade, answers } } })
        res.status(200).json({ newCourseList })
    } catch (err) {
        res.status(400).json({ error: err.message })
    }
...
const addTraineeInfo = async (req, res) => { // adds info for first time trainee
    try {
        ...
        const updatedTrainee = await trainee.findOneAndUpdate({ _id: id }, { name, gender }, { new: true, upsert: true })
...
const myCourses = async (req, res) => {
    try {
        const id = req.params.id
        const myCourseData = await trainee.findOne({ _id: id }, 'courseList.courseId courseList.progress -_id').lean()
